...
io.on('connection', async (socket) => {
    ...
    await redis.set(`socket:${socket.id}`, JSON.stringify({
        id: socket.id,
...
subscriber.subscribe('screenshot-completion', (err, count) => {
    if (err) {
        fastify.log.error('Failed to subscribe to screenshot-completion channel:', err);
        return;
    }
...
fastify.post('/screenshot', async (request, reply) => {
    ...
    const jobId = uuidv4();
    await channel.sendToQueue('screenshot_requests', Buffer.from(JSON.stringify({ 
        jobId, 
        url, 
        socketId,
        viewport
    })));
    return { message: 'Screenshot request received', jobId };
});
